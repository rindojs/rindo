...
/**
 * Modules that are polyfilled by Rindo, rather than using the default Node implementation.
 */
 ...
 * @returns the rollup plugin that resolves Rindo `sys` modules
 */
export function sysModulesPlugin(inputDir: string): Plugin {
  return {
    name: 'sysModulesPlugin',
    /**
     * A rollup build hook for resolving Rindo's environment agnostic packages

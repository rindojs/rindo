...
export async function compiler(opts: BuildOptions) {
  ...
  const compilerFileName = 'rindo.js';
  ...
  inMemoryFsDts = inMemoryFsDts.replace('@rindo/core/internal', '../../internal/index');
  ...
  transpileDts = transpileDts.replace('@rindo/core/internal', '../internal/index');
  ...
  const compilerBundle: RollupOptions = {
    ...
    output: {
      ...
      outro: cjsOutro,
      strict: false,
      banner: getBanner(opts, `Rindo Compiler`, true),
      esModule: false,
      preferConst: true,
      freeze: false,
      sourcemap: true,
    ...
    plugins: [
      ...
      terserPlugin(opts),
      {
        ...
        /**
         * A rollup build hook for resolving the Rindo mock-doc module, Microsoft's TypeScript event tracer, and the
      ...
      },
      {
        ...
        /**
         * A rollup build hook for loading the Rindo mock-doc module, Microsoft's TypeScript event tracer, the V8
...
async function minifyRindoCompiler(code: string, opts: BuildOptions) {
 ...
 * @param opts the Rindo build options, which includes the location of the TypeScript lib/

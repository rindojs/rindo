...
const BROWSER_COMPILER_DIST_FILE = path.join(ROOT_DIR, 'compiler', 'rindo.js');
...
async function bundleInternalClient() {
    ...
    plugins: [
      {
        resolveId(importee) {
          if (importee === '@build-conditionals') {
            return {
              id: '@rindo/core/internal/client/build-conditionals',
              external: true
            }
          }
  ...
  const { output } = await rollupBuild.generate({
    ...
    chunkFileNames: '[name].rindo-client.mjs'
  });

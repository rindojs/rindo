 ...
 *
 * @param rootDir the root of the Rindo repository
 */
...
/**
 * Validates the Rindo compiler. This includes verifying that the compiler, CLI and sys API can be instantiated,
...
async function validateCompiler(opts: BuildOptions): Promise<void> {
  const compilerPath = url.pathToFileURL(join(opts.output.compilerDir, 'rindo.js')).pathname;
...
async function validateTreeshaking(opts: BuildOptions) {
  ...
  await validateModuleTreeshake(opts, 'rindo-core', join(opts.output.internalDir, 'rindo-core', 'index.js'));
    ...
    plugins: [
      {
        name: 'rindoResolver',
        resolveId(id) {
          if (id === '@rindo/core/internal/client' || id === '@rindo/core') {
            return join(opts.output.internalDir, 'client', 'index.js');
          }
          if (id === '@rindo/core/internal/app-data') {
            return join(opts.output.internalDir, 'app-data', 'index.js');
          }
          if (id === '@rindo/core/internal/app-globals') {
            return id;
          }
...
/**
 * Represents a package/submodule of the bundled Rindo output to validate
 */

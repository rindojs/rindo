...
export async function buildDevServer(opts: BuildOptions) {
  ...
  const devServerIndexEsbuildOptions = {
    ...
    banner: {
      js: getBanner(opts, `Rindo Dev Server`, true),
    },
  ...
  const devServerProcessEsbuildOptions = {
    ...
    banner: {
      js: getBanner(opts, `Rindo Dev Server Process`, true),
    },
  } satisfies ESBuildOptions;

  const connectorAlias = {
    '@rindo/core/dev-server/client': join(inputDir, 'client', 'index.js'),
  };
  ...
  const devServerClientEsbuildOptions = {
    ...
    banner: {
      js: getBanner(opts, `Rindo Dev Server Client`, true),
    },
  } satisfies ESBuildOptions;
  ...
  return {
    ...
    setup(build) {
      build.onEnd(async (buildResult) => {
        ...
        code = code.replace(/__VERSION:RINDO__/g, opts.version);
...
const banner = `<!doctype html><html><head><meta charset="utf-8"><title>Rindo Dev Server Connector __VERSION:RINDO__ &#9889</title><style>body{background:black;color:white;font:18px monospace;text-align:center}</style></head><body>

Rindo Dev Server Connector __VERSION:RINDO__ &#9889;

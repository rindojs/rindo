...
export async function buildSysNode(opts: BuildOptions) {
  ...
  // write @rindo/core/sys/node/package.json
  ...
  const sysNodeAliases = {
    ...getEsbuildAliases(),
    '@rindo/core/compiler': '../../compiler/rindo.js',
  ...
  const sysNodeOptions: ESBuildOptions = {
    ...
    banner: { js: getBanner(opts, `Rindo Node System`, true) },
  ...
  const workerOptions: ESBuildOptions = {
    ...
    minify: true,
    alias: sysNodeAliases,
    banner: { js: getBanner(opts, `Rindo Node System Worker`, true) },
  };
...
async function sysNodeExternalBundles(opts: BuildOptions) {
  ...
  await Promise.all([
    ...
    // TODO(RINDO-1052): remove next two entries once Rollup -> esbuild migration is complete
   ...
   * TODO(RINDO-1323): remove once we deprecated Jest v26 support
   */
  ...
  // TODO(RINDO-1052): remove once Rollup -> esbuild migration is complete
  ...
  return new Promise<void>(async (resolveBundle, rejectBundle) => {
    ...
    const webpackConfig: Configuration = {
      ...
      externals(data, callback) {
        ...
        if (request === '@rindo/core/mock-doc') {

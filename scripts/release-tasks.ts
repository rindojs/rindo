...
/**
 * Runs a litany of tasks used to ensure a safe release of a new version of Rindo
...
export async function runReleaseTasks(opts: BuildOptions, args: ReadonlyArray<string>): Promise<void> {
  ...
  tasks.push(
    {
      ...
      // for actual releases, we'll need to build + bundle rindo in order to publish it to npm.
  ...
  if (opts.isPublishRelease) {
    tasks.push(
      {
        title: 'Publish @rindo/core to npm',
        task: () => {
          const cmd = 'npm';

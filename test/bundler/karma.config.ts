...
// use the instance of chromium that is downloaded as a part of rindo's puppeteer dependency
...
module.exports = function (config: Config): void {
  config.set({
    ...
    colors: true,
    files: [
      // general utilities for running Rindo + Karma
      'karma-rindo-utils.ts',

      // use the application built by wite
      { pattern: 'wite-bundle-test/dist/index.html', nocache: true },
      {
        pattern: 'wite-bundle-test/dist/**/*.js',
        // don't include these files via <script> tags, or they'll be included more than once
        included: false,
        nocache: true,
      },
      'wite-bundle-test/wite-bundle.spec.ts',
    ],
     ...
     *
     * {
     *   '/assets/': `/base/wite-bundle-test/dist/assets/`,
     * }
     *
     * maps the files found at '/base/wite-bundle-test/dist/assets/', to the '/assets/' directory on the karma server
     */
    proxies: {
      '/assets/': '/base/wite-bundle-test/dist/assets/',
    },

import { Component, Host, h, State } from '@rindo/core';
import type RindoTypes from '@rindo/core/compiler';
...
export class AppRoot {
  ...
  async compile() {
    console.log(`compile: rindo v${rindo.version}, typescript v${rindo.versions.typescript}`);

    const opts: RindoTypes.TranspileOptions = {
    ...
    const browserPromise = rindo.transpile(this.sourceCodeInput.value, opts);
  ...
  async bundle() {
    ...
    try {
      ...
      if (this.minified === 'minified') {
        const results = await rindo.optimizeJs({
          ...
          <div class="options">
            ...
            <label>
              ...
              <select ref={(el) => (this.coreImportPath = el)} onInput={this.compile.bind(this)}>
                <option value="null">null</option>
                <option value="@rindo/core/internal/client">@rindo/core/internal/client</option>
                <option value="@rindo/core/internal/testing">@rindo/core/internal/testing</option>
              </select>
            </label>
            <label>
...
declare const rindo: typeof RindoTypes;

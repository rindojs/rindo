...
type DomTestUtilities = {
   ...
   * @param waitForRindoReadyMs an optional period to wait for a Rindo application to be loaded, in milliseconds.
   * @returns the fully rendered HTML to test against
   */
  setupDom: (url: string, waitForRindoReadyMs?: number) => Promise<HTMLElement>;
  /**
...
/**
 * Helper function that keeps track of Rindo components who have fired the `componentWillRender` lifecycle event, but
 * have not fired the `componentDidRender` lifecycle event
 * @param elm the Rindo component that emitted the event
 */
...
/**
 * Helper function that keeps track of Rindo components who have fired the `componentDidRender` lifecycle event.
 * @param elm the Rindo component that emitted the event
 */
...
export function setupDomTests(document: Document): DomTestUtilities {
   ...
   */
  function setupDom(url: string, waitForRindoReady?: number): Promise<HTMLElement> {
    ...
    testElement.addEventListener('rindo_componentWillRender', (ev) => markWillRender(ev.target));
    testElement.addEventListener('rindo_componentDidRender', (ev) => didRender(ev.target));

    testElement.className = 'test-spec';
    if (!testBed) {
      console.error('The Rindo/Karma test bed could not be found.');
      process.exit(1);
    }

    testBed.appendChild(testElement);
    testElement.setAttribute('data-url', url);
    return renderTest(url, testElement, waitForRindoReady);
  }
   ...
   * @param waitForRindoReady the amount of time to wait (in milliseconds) to wait for an application to load
   * @returns the fully rendered HTML to test against
   */
  function renderTest(url: string, testElement: HTMLElement, waitForRindoReady?: number) {
    url = path.join('base', WWW_OUT_DIR, url);

    return new Promise<HTMLElement>((resolve, reject) => {
      try {
        /**
         * Ensure that all `onComponentReady` functions on Rindo elements in the DOM have been called before rendering
         * @returns an array of promises, one for each `onComponentReady` found on a Rindo component
         */
        const allReady = (): Promise<d.HTMLRindoElement[] | void> => {
          const promises: Promise<d.HTMLRindoElement>[] = [];
          /**
           * Function that recursively traverses the DOM, looking for Rindo components. Any `componentOnReady`
           * functions found on Rindo components are pushed to a buffer to be run after traversing the entire DOM.
          ...
          const waitForDidLoad = (elm: Element): void => {
            if (elm != null && elm.nodeType === 1) {
              ...
              for (let i = 0; i < elm.children.length; i++) {
                ...
                if (childElm.tagName.includes('-') && isHtmlRindoElement(childElm)) {
        ...
        const onIndexHtmlLoaded = function (this: XMLHttpRequest): void {
          ...
          if (typeof waitForRindoReady === 'number') {
            setTimeout(() => {
              resolve(testElement);
            }, waitForRindoReady);
          } else {
            /**
             * Create a listener for Rindo's "appload" event to signal to the test framework the application and its
...
/**
 * Type guard to verify some entity is an instance of Rindo HTML Element
 * @param elm the entity to test
 * @returns `true` if the entity is a Rindo HTML Element, `false` otherwise
 */
function isHtmlRindoElement(elm: unknown): elm is d.HTMLRindoElement {

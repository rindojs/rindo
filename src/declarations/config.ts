...
import { DevServerConfig, RindoDevServerConfig } from './dev-server';
import { Logger } from './logger';
import { OutputTarget } from './output-targets';
import { RindoSystem } from './system';
...
export interface RindoConfig {
   ...
   * Notice that Rindo always generates a modern build too, this setting will just disable the additional `es5` build.
   */
  buildEs5?: boolean;
  ...
  /**
   * Sets if the CSS is minified or not. Rindo uses `cssnano` under the hood.
   * Defaults to `false` in dev mode and `true` in production mode.
   */
  minifyCss?: boolean;
...
export interface BrowserConfig extends RindoConfig {
  win?: Window;
}

...
describe('sourcemaps', () => {
  describe('rollupToRindoSourceMap', () => {
    it('returns null if the given sourcemap is null', () => {
      expect(rollupToRindoSourceMap(null)).toBeNull();
    });

    it('returns null if the given sourcemap is undefined', () => {
      expect(rollupToRindoSourceMap(undefined)).toBeNull();
    });

    it('transforms a rollup sourcemap to a rindo sourcemap', () => {
      const rollupSourceMap: RollupSourceMap = {
        ...
        sourcesContent: [
          '/*\\n Rindo Client Patch Esm v0.0.0-dev.20210825190806 | MIT Licensed | https://rindojs.web.app\\n */',
        ],
        version: 3,
        toString: () => 'stub',
        toUrl: () => 'stub',
      ...
      const rindoSourceMap = rollupToRindoSourceMap(rollupSourceMap);
      ...
      expect(rindoSourceMap).toEqual(expectedSourceMap);
    });
  });

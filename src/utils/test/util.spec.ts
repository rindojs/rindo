import { mockBuildCtx, mockValidatedConfig } from '@rindo/core/testing';
...
describe('util', () => {
  describe('generatePreamble', () => {
    it('generates a comment with a single line preamble', () => {
      const testConfig = mockValidatedConfig({ preamble: 'I am Rindo' });
 ...
 */`);
  ...
  describe('hasDependency', () => {
    ...
    it("returns false for '@rindo/core'", () => {
      buildCtx.packageJson.dependencies = { '@rindo/core': '2.0.0' };

      expect(util.hasDependency(buildCtx, '@rindo/core')).toBe(false);
    });
  ...
  it('createJsVarName', () => {
    ...
    expect(util.createJsVarName('./scoped-style-import.css=data')).toBe('scopedStyleImportCss');
    expect(util.createJsVarName('@familyjs/core')).toBe('familyjsCore');
    expect(util.createJsVarName('@familyjs\\core')).toBe('familyjsCore');
  ...
  describe('addDocBlock', () => {
    ...
    beforeEach(() => {
      str = 'interface Foo extends Components.Foo, HTMLRindoElement {';
      docs = {
 ...
 */
interface Foo extends Components.Foo, HTMLRindoElement {`);
    });

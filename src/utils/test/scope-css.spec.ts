...
describe('ShadowCss', function () {
  ...
  it('div', () => {
    const r = s('div {}', 'sc-fml-tag', true);
    expect(r).toEqual('/*!@div*/div.sc-fml-tag {}');
  });
  ...
  describe('::slotted', () => {
    it('should handle *, commentOriginalSelector', () => {
      const r = s('::slotted(*) {}', 'sc-fml-tag', true);
      expect(r).toEqual('/*!@::slotted(*)*/.sc-fml-tag-s > * {}');
    });

    it('should handle *', () => {
      const r = s('::slotted(*) {}', 'sc-fml-tag');
      expect(r).toEqual('.sc-fml-tag-s > * {}');
    });

    it('should handle * descendant', () => {
      const r = s('::slotted(*) .my-class {}', 'sc-fml-tag');
      expect(r).toEqual('.sc-fml-tag-s .my-class {}');
    });

    it('should handle :host complex selector', () => {
      const r = s(':host > ::slotted(*:nth-of-type(2n - 1)) {}', 'sc-fml-tag');
      expect(r).toEqual(
        '.sc-fml-tag-h >.sc-fml-tag-s > *:nth-of-type(2n - 1), .sc-fml-tag-h > .sc-fml-tag-s > *:nth-of-type(2n - 1) {}',
      );
    });

    it('should handle host-context complex selector', () => {
      const r = s(':host-context(.red) > ::slotted(*:nth-of-type(2n - 1)) {}', 'sc-fml-tag');
      expect(r).toEqual(
        '.sc-fml-tag-h.red >.sc-fml-tag-s > *:nth-of-type(2n - 1), .sc-fml-tag-h.red > .sc-fml-tag-s > *:nth-of-type(2n - 1), .red .sc-fml-tag-h >.sc-fml-tag-s > *:nth-of-type(2n - 1), .red .sc-fml-tag-h > .sc-fml-tag-s > *:nth-of-type(2n - 1) {}',
      );
    });

    it('should handle left side selector', () => {
      const r = s('div::slotted(ul) {}', 'sc-fml-tag');
      expect(r).toEqual('div.sc-fml-tag-s > ul {}');
    });

    it('should handle tag selector', () => {
      const r = s('::slotted(ul) {}', 'sc-fml-tag');
      expect(r).toEqual('.sc-fml-tag-s > ul {}');
    });

    it('should handle class selector', () => {
      const r = s('::slotted(.foo) {}', 'sc-fml-tag');
      expect(r).toEqual('.sc-fml-tag-s > .foo {}');
    });

    it('should handle multiple selector', () => {
      const r = s('::slotted(ul), ::slotted(li) {}', 'sc-fml-tag');
      expect(r).toEqual('.sc-fml-tag-s > ul, .sc-fml-tag-s > li {}');
    });

    it('should combine parent selector', () => {
      const r = s('div{} .a .b .c ::slotted(*) {}', 'sc-fml-tag');
      expect(r).toEqual('div.sc-fml-tag{} .a .b .c.sc-fml-tag-s > *, .a .b .c .sc-fml-tag-s > * {}');
    });

    it('same selectors', () => {
      const r = s('::slotted(*) {}, ::slotted(*) {}, ::slotted(*) {}', 'sc-fml-tag');
      expect(r).toEqual('.sc-fml-tag-s > * {}, .sc-fml-tag-s > * {}, .sc-fml-tag-s > * {}');
    });

    it('same selectors, commentOriginalSelector', () => {
      const r = s('::slotted(*) {}, ::slotted(*) {}, ::slotted(*) {}', 'sc-fml-tag', true);
      expect(r).toEqual(
        '/*!@::slotted(*)*/.sc-fml-tag-s > * {}/*!@, ::slotted(*)*/.sc-fml-tag, .sc-fml-tag-s > * {}/*!@, ::slotted(*)*/.sc-fml-tag, .sc-fml-tag-s > * {}',
      );
    });

    it('should combine parent selector when comma', () => {
      const r = s('.a .b, .c ::slotted(*) {}', 'sc-fml-tag');
      expect(r).toEqual('.a.sc-fml-tag .b.sc-fml-tag, .c.sc-fml-tag-s > *, .c .sc-fml-tag-s > * {}');
    });

    it('should handle multiple selector, commentOriginalSelector', () => {
      const r = s('::slotted(ul), ::slotted(li) {}', 'sc-fml-tag', true);
      expect(r).toEqual('/*!@::slotted(ul), ::slotted(li)*/.sc-fml-tag-s > ul, .sc-fml-tag-s > li {}');
    });
  ...
  describe('convertScopedToShadow', () => {
    ...
    it('div', () => {
      const input = `/*!@div*/div.sc-fml-tag {}`;
      const expected = `div{}`;
      expect(convertScopedToShadow(input)).toBe(expected);
    });
    ...
    it('::slotted', () => {
      const input = `/*!@::slotted(ul), ::slotted(li)*/.sc-fml-tag-s > ul, .sc-fml-tag-s > li {}`;

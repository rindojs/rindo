...
/**
 * Get configuration necessary to carry out transpilation, including a Rindo
 * configuration, transformation options, and transpilation options.
 *
 * @param input options for Rindo's transpiler (string-to-string compiler)
...
export const getTranspileConfig = (input: TranspileOptions): TranspileConfig => {
  ...
  const compileOpts: TranspileOptions = {
    ...
    coreImportPath: isString(input.coreImportPath) ? input.coreImportPath : RINDO_INTERNAL_CLIENT_ID,

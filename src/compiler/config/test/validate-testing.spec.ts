...
import { mockCompilerSystem, mockLoadConfigInit, mockLogger } from '@rindo/core/testing';
...
describe('validateTesting', () => {
  ...
  beforeEach(() => {
    ...
    userConfig = {
      ...
      // use Node's join() here to simulate a user using either Win/Posix separators (depending on the platform these
      // tests are run on) for their input
      configPath: path.join(ROOT, 'User', 'some', 'path', 'rindo.config.ts'),
    };
  ...
  describe('testPathIgnorePatterns', () => {
    ...
    it('sets the default testPathIgnorePatterns if no array is provided', () => {
      ...
      expect(config.testing.testPathIgnorePatterns).toEqual([
        ...
        join(ROOT, 'User', 'some', 'path', '.rindo'),
  ...
  describe('preset', () => {
    it.each([null, true])("uses rindo's default preset if a non-string (%s) is provided", (nonStringPreset) => {
      userConfig.flags = { ...flags, e2e: true };
      userConfig.testing = {
  ...
  describe('testRegex', () => {
    ...
    beforeEach(() => {
      ...
      if (!testRegexSetting) {
        throw new Error('No testRegex was found in the Rindo TestingConfig. Failing test.');
      }

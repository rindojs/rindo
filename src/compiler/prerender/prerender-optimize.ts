...
export const excludeStaticComponents = (
  ...
  if (staticComponents.length > 0) {
    const rindoScriptElm = doc.querySelector('script[data-rindo-namespace]');
    if (rindoScriptElm) {
      const namespace = rindoScriptElm.getAttribute('data-rindo-namespace');
      ...
      rindoScriptElm.parentNode.insertBefore(dataOptsScript, rindoScriptElm.nextSibling);
    }
  }
};
...
export const removeRindoScripts = (doc: Document) => {
  const rindoScripts = doc.querySelectorAll('script[data-rindo]');
  for (let i = rindoScripts.length - 1; i >= 0; i--) {
    rindoScripts[i].remove();
  }
};
...
export const hashAssets = async (
  ...
  const pageStates = Array.from(
    doc.querySelectorAll('script[data-rindo-static="page.state"][type="application/json"]'),

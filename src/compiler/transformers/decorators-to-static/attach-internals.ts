 ...
 * Rindo components rich participants in whatever `HTMLFormElement` instances
...
export const attachInternalsDecoratorsToStatic = (
  ...
  if (attachInternalsMembers.length > 1) {
    ...
    error.messageText = `Rindo does not support adding more than one AttachInternals() decorator to a component`;
    return;
  }

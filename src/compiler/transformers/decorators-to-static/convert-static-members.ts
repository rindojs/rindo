 ...
 * - The property declaration does not include a Rindo @Prop or @State decorator
...
export const hasStaticInitializerInClass = (classElm: ts.ClassElement): boolean => {
  return (
    ...
    !classElm.modifiers!.some(isRindoStateOrPropDecorator)
  );
};

/**
 * Determines if a Modifier-like node is a Rindo `@Prop()` or `@State()` decorator
 ...
 */
const isRindoStateOrPropDecorator = (modifier: ts.ModifierLike): boolean => {

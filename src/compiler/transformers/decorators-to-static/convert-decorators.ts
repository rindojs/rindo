 ...
 * decorators on Rindo component classes (i.e. classes decorated with
 * `@Component`) to static representations of the options, names, etc.
 * associated with the various decorators that Rindo supports like `@Prop`,
 * `@State`, etc.
 *
 * This `TransformerFactory` returned by this function will handle all classes
 * declared within a module.
 *
 * @param config a user-supplied configuration for Rindo
 ...
 * 2. Strip off properties decorated with Rindo-implemented decorators like
 ...
 * the rest of the Rindo compilation pipeline (e.g. in output target
 * generation).
 *
 * @param config a user-supplied Rindo config
 * @param diagnostics for surfacing errors and warnings
 * @param typeChecker a TypeScript typechecker instance
 * @param program a {@link ts.Program} object
...
const visitClassDeclaration = (
  ...
  // with a Rindo decorator. We do this here because we'll implement the
  // behavior specified for those decorated methods later on.
  const filteredMethodsAndFields = removeRindoMethodDecorators(Array.from(classMembers), diagnostics, importAliasMap);
 ...
 * method elements which are Rindo-specific decorators. We implement the
 * intended behavior for these Rindo-specific decorators (things like
 * `@Watch`, `@State`, etc) through a combination of compile- and
 * run-time changes, scaffolding, etc.
 *
 * This utility modifies these class elements to remove any Rindo-specific
 * decorators.
 *
 ...
 * may be added
 * @param importAliasMap a map of Rindo decorator names to their import names
 * @returns a new list of the same ClassElement nodes, with any nodes which have
 * Rindo-specific decorators modified to remove them
 */
const removeRindoMethodDecorators = (
...
/**
 * This updates a Rindo component class declaration AST node to handle any
 * class fields with Rindo-specific decorators (`@State`, `@Prop`, etc). For
 ...
 *
 * When a class field is decorated with a Rindo-defined decorator, we rely on
 ...
 * that the TypeScript configuration used for Rindo have the
 ...
 * decorated with a Rindo decorator would also be instantiated in the
 * constructor.
 *
 * So instead, we take matters into our own hands. When we encounter a class
 * field which is decorated with a Rindo decorator we remove it from the
 ...
 *
 * @param classNode a TypeScript AST node for a Rindo component class
...
function handleClassFields(
  ...
  for (const member of classMembers) {
    if (shouldInitializeInConstructor(member, importAliasMap) && ts.isPropertyDeclaration(member)) {
      ...
      // if it's not a class field that is decorated with a Rindo decorator then

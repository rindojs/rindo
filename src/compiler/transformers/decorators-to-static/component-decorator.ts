...
/**
 * Perform code generation to create new class members for a Rindo component
...
/**
 * Perform validation on a Rindo component in preparation for some
...
const validateComponent = (
  ...
  if (extendNode) {
    ...
    Rindo needs to be able to switch between different base classes in order to implement the different output targets such as: lazy and raw web components.`;
  ...
  if (otherDecorator) {
    ...
    Rindo performs extensive static analysis on top of your components in order to generate the necessary metadata, runtime decorators at the components level make this task very hard.`;
  ...
  if (!config._isTesting) {
    ...
    nonTypeExports.forEach((symbol) => {
      ...
      For further information check out: https://rindojs.web.app/docs/module-bundling`;

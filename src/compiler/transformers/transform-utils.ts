...
import { MEMBER_DECORATORS_TO_REMOVE, RindoStaticGetter } from './decorators-to-static/decorators-constants';
...
export const validateReferences = (
  ...
  Object.keys(references).forEach((refName) => {
    ...
    if (ref.path === '@rindo/core' && readOnlyArrayHasStringMember(MEMBER_DECORATORS_TO_REMOVE, refName)) {
...
export const resolveType = (checker: ts.TypeChecker, type: ts.Type): string => {
  ...
  // TODO(RINDO-366): Get this section of code under tests that directly exercise this behavior
  if (parts.length > 1) {
...
/**
 * Maps a TypeScript 4.3+ JSDocTagInfo to a flattened Rindo CompilerJsDocTagInfo.
...
export const serializeDocsSymbol = (checker: ts.TypeChecker, symbol: ts.Symbol) => {
  ...
  // TODO(RINDO-365): Replace this with `return resolveType()`;
  const set = new Set<string>();
  parseDocsType(checker, type, set);

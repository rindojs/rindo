...
const BLACKLISTED_COMPONENT_METHODS = [
   ...
   * this would cause issues when Rindo tries to hydrate the component.
   */
  'shadowRoot',
];
...
const validateComponentMembers = (node: ts.Node) => {
  ...
  if (
    ...
    /**
     * the class is actually a Rindo component, has a decorator with a property named "tag"
     */
    if (
      ...
      throw new Error(
        `The component "${componentName}" has a getter called "${propName}". This getter is reserved for use by Rindo components and should not be defined by the user.`,
      );
    }
  }
};

 ...
 * properties to all of the JSX nodes contained inside of a Rindo component's
 * `render` function.
 *
...
export const performAutomaticKeyInsertion = (transformCtx: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
   ...
   * declaration which is also a Rindo component, at which point it hands
  ...
  function findClassDeclVisitor(node: ts.Node): ts.VisitResult<ts.Node> {
    if (ts.isClassDeclaration(node)) {
      ...
      if (tagName != null) {
        ...
        // the class we're dealing with is a Rindo component which has
    ...
    // component tag name, so this is not a rindo component!
   ...
   * on a Rindo class and then passing off responsibility to the inner-most
  ...
  function findRenderMethodVisitor(node: ts.Node): ts.VisitResult<ts.Node> {
    ...
    // ```
    //
    // Since the `<div>` tags don't have `key` attributes the Rindo vdom will

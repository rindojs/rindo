...
export function validateCollectionCompatibility(config: d.Config, collection: d.Collection): number[] {
  ...
  collection.compiler.name = collection.compiler.name || '@rindo/core';
...
export function calculateRequiredUpgrades(config: d.Config, collectionVersion: string) {
  ...
  if (config.sys.semver.lte(collectionVersion, '0.2.0')) {
    // 2018-01-19
    // ensure all @rindo/core imports are removed
    upgrades.push(CompilerUpgrade.Remove_Rindo_Imports);
  }

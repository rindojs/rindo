...
import {
  ...
  RINDO_CORE_ID,
  RINDO_INTERNAL_CLIENT_PATCH_ID,
  USER_INDEX_ENTRY_ID,
...
const getLazyEntry = (isBrowser: boolean) => {
  ...
  s.append(`import { bootstrapLazy } from '${RINDO_CORE_ID}';\n`);

  if (isBrowser) {
    s.append(`import { patchBrowser } from '${RINDO_INTERNAL_CLIENT_PATCH_ID}';\n`);
    ...
    s.append(`import { patchEsm } from '${RINDO_INTERNAL_CLIENT_PATCH_ID}';\n`);
    s.append(`import { globalScripts } from '${RINDO_APP_GLOBALS_ID}';\n`);
...
(function(doc){
  ...
  scriptElm.setAttribute('data-rindo-namespace', '${namespace}');

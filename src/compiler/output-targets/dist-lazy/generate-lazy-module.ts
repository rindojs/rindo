...
/**
 * Add imports for each bundle to Rindo's lazy loader. Some bundlers that are built atop of Rollup strictly impose
 ...
 * @param rollupChunkResults the results of running Rollup across a Rindo project
...
const addStaticImports = (rollupChunkResults: d.RollupChunkResult[], bundleModules: d.BundleModule[]): void => {
  rollupChunkResults.filter(isRindoCoreResult).forEach((index: d.RollupChunkResult) => {
...
/**
 * Determine if a Rollup output chunk contains Rindo runtime code
 * @param rollupChunkResult the rollup chunk output to test
 * @returns true if the output chunk contains Rindo runtime code, false otherwise
 */
const isRindoCoreResult = (rollupChunkResult: d.RollupChunkResult): boolean => {
  return (
...
const writeLazyEntry = async (
  ...
  const inputCode = rollupResult.code.replace(`[/*!__RINDO_LAZY_DATA__*/]`, `${lazyRuntimeData}`);
  const { code, sourceMap } = await convertChunk(
    config,
    compilerCtx,
    buildCtx,
...
/**
 * Sorts, formats, and stringifies the bundles for a lazy build of a Rindo project.
 *
 * @param bundleModules The modules for the Rindo lazy build emitted from Rollup.
...
const convertChunk = async (
  ...
  let sourceMap = rollupToRindoSourceMap(rollupSrcMap);

    ...
    if (build) {
      ...
      const files = rollupOutput.output.map(async (bundle) => {
        if (bundle.type === 'chunk') {
          ...
          let sourceMap = bundle.map ? rollupToRindoSourceMap(bundle.map) : undefined;
...
export const addCustomElementInputs = (
  ...
  components.forEach((cmp) => {
    ...
    if (cmp.isPlain) {
      ...
      // TODO(RINDO-855): Invalid syntax generation, note the unbalanced left curly brace
 ...
 * `dist-custom-elements` output target, that will be applied to a Rindo

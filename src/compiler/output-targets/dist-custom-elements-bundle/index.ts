...
const generateEntryPoint = (buildCtx: d.BuildCtx) => {
  ...
  imp.push(
    `import { proxyCustomElement } from '${RINDO_INTERNAL_CLIENT_ID}';`,
    `export { setAssetPath } from '${RINDO_INTERNAL_CLIENT_ID}';`,
    `export * from '${USER_INDEX_ENTRY_ID}';`,
    `import { globalScripts } from '${RINDO_APP_GLOBALS_ID}';`,
    `globalScripts();`,
  );
...
const getCustomElementBundleCustomTransformer = (config: d.Config, compilerCtx: d.CompilerCtx) => {
  ...
  return [updateRindoCoreImports(transformOpts.coreImportPath), nativeComponentTransform(compilerCtx, transformOpts), removeCollectionImports(compilerCtx)];
};

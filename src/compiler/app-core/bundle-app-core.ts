...
import { rindoBuildConditionalsPlugin } from '../rollup-plugins/rindo-build-conditionals';
import { rindoClientPlugin } from '../rollup-plugins/rindo-client';
import { rindoExternalRuntimePlugin } from '../rollup-plugins/rindo-external-runtime';
...
export const bundleApp = async (config: d.Config, compilerCtx: d.CompilerCtx, buildCtx: d.BuildCtx, build: d.Build, bundleAppOptions: d.BundleAppOptions) => {
  ...
  try {
    ...
    const rollupOptions: RollupOptions = {
      ...
      plugins: [
        rindoExternalRuntimePlugin(bundleAppOptions.externalRuntime),
        loaderPlugin({
          '@rindo/core': DEFAULT_CORE,
          '@core-entrypoint': DEFAULT_ENTRY,
          ...bundleAppOptions.loader
        }),
        rindoClientPlugin(config),
        rindoBuildConditionalsPlugin(build, config.fsNamespace),
...
export const generateRollupOutput = async (build: RollupBuild, options: OutputOptions, config: d.Config, entryModules: d.EntryModule[]): Promise<d.RollupResult[]> => {
  ...
  return output
    ...
    .map((chunk: OutputChunk) => {
      const isCore = Object.keys(chunk.modules).includes('@rindo/core');
      return {
...
export * from '@rindo/core/platform';
import globals from '@rindo/core/global-scripts';
export { globals };
`;

export const DEFAULT_ENTRY = `
export * from '@rindo/core';
`;

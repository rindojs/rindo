 ...
 * the compiler when running a build in watch mode (i.e. `rindo build --watch`).
 *
 * In essence, this method tracks all files that change while the program is running to trigger
 * a rebuild of a Rindo project using a {@link ts.EmitAndSemanticDiagnosticsBuilderProgram}.
 *
 * @param config The validated config for the Rindo project
...
export const createWatchBuild = async (
  ...
  /**
   * A callback function that is invoked to trigger a rebuild of a Rindo project. This will
   * update the build context with the associated file changes (these are used downstream to trigger
   * HMR) and then calls the `build()` function to execute the Rindo build.
   *
   ...
   *
   * @param filePath The absolute path to the file in the Rindo project
...
/**
 * Recursively marks all files under a Rindo project's `src` directory to be watched for changes. Whenever
...
/**
 * Marks all root files of a Rindo project to be watched for changes. Whenever
 * one of these files is determined as changed (according to TS), a rebuild of the project will execute.
 *
 * @param config The Rindo project's config
 * @param compilerCtx The compiler context for the Rindo project

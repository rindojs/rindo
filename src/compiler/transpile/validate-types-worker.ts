...
import { removeRindoDecorators, removeRindoImports } from '../transformers/validate-types-transform';
  ...
  if (!workerCtx.tsHost) {
    ...
    tsHost.writeFile = (outputFileName: string, data: string, writeByteOrderMark: boolean) => {
      ...
      if (typeof compilerOptions.declarationDir === 'string') {
        data = updateRindoTypesImports(path, compilerOptions.declarationDir, outputFileName, data);
      }

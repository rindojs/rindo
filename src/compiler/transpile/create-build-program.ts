...
/**
 * Create a TypeScript Program ({@link ts.Program}) to perform builds of a Rindo project using the provided
 * `buildCallback` entity
 * @param config a Rindo configuration to apply to a full build of a Rindo project
 * @param buildCallback a callback that invokes the actual transpilation of a Rindo project
 * @returns a Program that marries the TypeScript and Rindo compilers together.
 */
export const createTsBuildProgram = async (
  ...
  const tsWatchSys: ts.System = {
     ...
     * @param callback a callback to invoke upon the completion of compilation. this function is provided to Rindo by
   ...
   * {@link ts.WatchCompilerHostOfConfigFile} class has the following features that makes it useful to Rindo (even
   * when Rindo is performing a single, full build):
   ...
   * - it includes the {@link ts.WatchCompilerHost#afterProgramCreate} function in its interface, which Rindo
  ...
  /**
   * Create the initial {@link ts.Program} using Rindo's custom {@link ts.WatchCompilerHostOfConfigFile}. The Program
   * represents the _TypeScript_ compiler context, that will work in tandem with Rindo's compiler context and build
   * context
   */

...
import { updateRindoTypesImports } from '../types/rindo-types';
  ...
  const emitCallback: ts.WriteFileCallback = (emitFilePath, data, _w, _e, tsSourceFiles) => {
    if (emitFilePath.endsWith('.js') || emitFilePath.endsWith('js.map')) {
      ...
      typesOutputTarget.forEach((o) => {
        ...
        data = updateRindoTypesImports(o.typesDir, distPath, data);
      ...
      .map((srcRootDtsFilePath) => {
        ...
        return Promise.all(
          typesOutputTarget.map(async (o) => {
            ...
            dtsContent = updateRindoTypesImports(o.typesDir, distPath, dtsContent);
 ...
 * within the Rindo project's source directory.
 *
 ...
 *
 * @param config a Rindo configuration object

...
export function generateComponentTypes(cmp: d.ComponentCompilerMeta): d.TypesModule {
  ...
  const rindoComponentAttributes = attributesToMultiLineString([
    ...propAttributes,
    ...methodAttributes
  ], false);
  const isDep = cmp.isCollectionDependency;
  const rindoComponentJSXAttributes = attributesToMultiLineString([
    ...propAttributes,
    ...eventAttributes
  ], true);
  return {
    ...
    tagNameAsPascal,
    htmlElementName,
    component: `interface ${tagNameAsPascal} {${rindoComponentAttributes}}`,
    jsx: `interface ${tagNameAsPascal} {${rindoComponentJSXAttributes}}`,
    element: `

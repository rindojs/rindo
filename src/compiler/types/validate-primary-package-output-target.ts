...
export type PrimaryPackageOutputTargetRecommendedConfig = {
   ...
   *
   * @param rootDir The Rindo project's root directory pulled from the validated config.
...
/**
 * Performs validation for specified fields in a Rindo project's
...
export const validatePrimaryPackageOutputTarget = (
  ...
  if (config.validatePrimaryPackageOutputTarget) {
      ...
      if (targetsMarkedToValidate.length) {
        ...
        if (targetsMarkedToValidate.length > 1) {
          logValidationWarning(
            buildCtx,
            `Your Rindo config has multiple output targets with 'isPrimaryPackageOutputTarget: true'. Rindo does not support validating 'package.json' fields for multiple output targets. Please remove the 'isPrimaryPackageOutputTarget' flag from all but one of the following output targets: ${targetsMarkedToValidate
              .map((ref) => ref.type)
              .join(
                ', ',
              )}. For now, Rindo will use the first primary target it finds. You can read more about primary package output targets in the Rindo docs: https://rindojs.web.app/docs/output-targets#primary-package-output-target-validation`,
          );
        }
        ...
        logValidationWarning(
          buildCtx,
          `Your Rindo project has not assigned a primary package output target. Rindo recommends that you assign a primary output target so it can validate values for fields in your project's 'package.json'. You can read more about primary package output targets in the Rindo docs: https://rindojs.web.app/docs/output-targets#primary-package-output-target-validation`,
        );
      }
    }
    ...
    if (nonPrimaryTargets.length && nonPrimaryTargets.some((ref: any) => ref.isPrimaryPackageOutputTarget)) {
      logValidationWarning(
        buildCtx,
        `Your Rindo project has assigned one or more ineligible output targets as the primary package output target. No validation will take place. Please remove the 'isPrimaryPackageOutputTarget' flag from the following output targets in your Rindo config: ${nonPrimaryTargets
          .filter((ref: any) => ref.isPrimaryPackageOutputTarget === true)
          .map((ref) => ref.type)
          .join(
            ', ',
          )}. You can read more about primary package output targets in the Rindo docs: https://rindojs.web.app/docs/output-targets#primary-package-output-target-validation`,
      );
    }
  } else {
    if (config.outputTargets.some((ref: any) => ref.isPrimaryPackageOutputTarget)) {
      logValidationWarning(
        buildCtx,
        'Your Rindo project has designated a primary package output target without enabling primary package validation for your project. Either set `validatePrimaryPackageOutputTarget: true` in your Rindo config or remove `isPrimaryPackageOutputTarget: true` from all output targets. You can read more about primary package output targets in the Rindo docs: https://rindojs.web.app/docs/output-targets#primary-package-output-target-validation',
      );
    }
  }
};
...
/**
 * Validates the `module` field in a Rindo project's `package.json`. This function performs
 * basic checks for a value to be set for `module` as well as checks that the specified path
 * matches Rindo's recommended value based on the output target the user designated to be used
 ...
 * @param compilerCtx The project's compiler context.
 * @param buildCtx The project's build context.
 * @param recommendedOutputTargetConfig The config object containing the function to generate the Rindo
...
/**
 * Validates the `types` field in a Rindo project's `package.json`. This function performs

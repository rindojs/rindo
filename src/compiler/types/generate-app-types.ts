...
/**
 * Generates a `components.d.ts` file's contents, which contains the typings for all components in a Rindo project
 * @param config the Rindo configuration associated with the project being compiled
 * @param buildCtx the context associated with the current build
...
const generateComponentTypesFile = (config: d.Config, buildCtx: d.BuildCtx, areTypesInternal: boolean): string => {
  ...
  const modules: d.TypesModule[] = components.map((cmp) => {
     ...
     * data structure for each Rindo component in series, therefore the memory footprint of this entity will likely

...
function checkPluginHelper(config: d.Config, buildCtx: d.BuildCtx, externalStylePath: string, pluginExts: string[], pluginId: string, pluginName: string) {
  ...
  const msg = [
    ...
    `plugin has not been installed. Please install the "@rindo/${pluginId}" `,
    `plugin and add it to "config.plugins" within the project's rindo config `,
    `file. For more info please see: https://www.npmjs.com/package/@rindo/${pluginId}`
  ].join('');

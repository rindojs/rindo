...
import { isDtsFile, isJsFile, isJsxFile, isLocalModule, isRindoCoreImport, isTsxFile, isTsFile, isJsonFile } from '../resolve/resolve-utils';
...
export const tsResolveNodeModule = (config: d.Config, inMemoryFs: d.InMemoryFileSystem, moduleId: string, containingFile: string): ts.ResolvedModuleWithFailedLookupLocations => {
  if (isRindoCoreImport(moduleId)) {
    const rtn: ts.ResolvedModuleWithFailedLookupLocations = {
      resolvedModule: {
        extension: ts.Extension.Dts,
        resolvedFileName: normalizePath(config.sys.getLocalModulePath({ rootDir: config.rootDir, moduleId: '@rindo/core', path: 'internal/index.d.ts' })),
        packageId: {
          name: moduleId,
          subModuleName: '',
          version,

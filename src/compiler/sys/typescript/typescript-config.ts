    ...
    if (!readTsConfig) {
      ...
      if (results.errors && results.errors.length > 0) {
        ...
        if (results.raw) {
          ...
          if (hasRindoConfigInclude(results.raw.include)) {
            const warn = buildWarn(tsconfig.diagnostics);
            warn.header = `tsconfig.json should not reference rindo.config.ts`;
            warn.messageText = `rindo.config.ts is not part of the output build, it should not be included.`;
          }
...
const hasRindoConfigInclude = (includeProp: string[]) => Array.isArray(includeProp) && includeProp.includes('rindo.config.ts');

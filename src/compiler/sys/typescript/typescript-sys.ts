...
export const patchTsSystemFileSystem = (config: d.Config, rindoSys: d.CompilerSystem, inMemoryFs: d.InMemoryFileSystem, loadedTs: typeof ts, tsSys: ts.System) => {
  ...
  tsSys.createDirectory = p => {
    rindoSys.mkdirSync(p, { recursive: true });
  };
...
const patchTsSystemWatch = (rindoSys: d.CompilerSystem, loadedTs: typeof ts, tsSys: ts.System) => {
  tsSys.watchDirectory = (p, cb, recursive) => {
    const watcher = rindoSys.watchDirectory(
      p,
      filePath => {
        cb(filePath);
      },

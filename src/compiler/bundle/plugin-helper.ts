  ...
  return {
    ...
    resolveId(importee: string, importer: string): null {
      ...
      if (builtIns.has(importee)) {
        ...
        diagnostic.messageText = `For the import "${importee}" to be bundled${fromMsg}, ensure the "rollup-plugin-node-polyfills" plugin is installed and added to the rindo config plugins (${platform}). Please see the bundling docs for more information.
        Further information: https://rindojs.web.app/docs/module-bundling`;
      }
      return null;
    },
  };

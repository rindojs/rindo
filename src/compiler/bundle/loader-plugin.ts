...
/**
 * Rollup plugin that aids in resolving the entry points (1 or more files) for a Rindo project. For example, a project
...
export const loaderPlugin = (entries: { [id: string]: string } = {}): Plugin => {
  return {
    ...
    /**
     * A rollup build hook for resolving the imports of individual Rindo project files. This hook only resolves
    ...
    /**
     * A rollup build hook for loading individual Rindo project files [Source](https://rollupjs.org/guide/en/#load)

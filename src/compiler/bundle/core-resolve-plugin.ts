...
export const coreResolvePlugin = (config: d.Config, compilerCtx: d.CompilerCtx, platform: 'client' | 'hydrate' | 'worker', externalRuntime: boolean): Plugin => {
  if (platform === 'worker') {
    return {
      ...
      resolveId(id) {
        if (id === RINDO_CORE_ID || id === RINDO_INTERNAL_CLIENT_ID || id === RINDO_INTERNAL_HYDRATE_ID) {
  ...
  const internalClientPatch = getRindoInternalModule(config, compilerExe, 'client/patch.js');
  ...
  return {
    ...
    resolveId(id) {
      ...
      if (id === RINDO_INTERNAL_CLIENT_PATCH_ID) {
        if (externalRuntime) {
          return {
            id: RINDO_INTERNAL_CLIENT_PATCH_ID,
            external: true,
          };
        }

...
import { APP_DATA_CONDITIONAL, RINDO_APP_DATA_ID, RINDO_APP_GLOBALS_ID } from './entry-alias-ids';
...
export const appDataPlugin = (
  ...
  return {
    ...
    resolveId(id: string, importer: string | undefined): ResolveIdResult {
      if (id === RINDO_APP_DATA_ID || id === RINDO_APP_GLOBALS_ID) {
        if (platform === 'worker') {
          this.error('@rindo/core packages cannot be imported from a worker.');
        }

        if (platform === 'hydrate' || RINDO_APP_GLOBALS_ID) {
...
/**
 * Generates the `BUILD` constant that is used at compile-time in a Rindo project
 *

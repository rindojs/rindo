...
export const patchEsm = () => {
  ...
  if (BUILD.cssVarShim && !(CSS && CSS.supports && CSS.supports('color', 'var(--c)'))) {
    // @ts-ignore
    return import(/* webpackChunkName: "rindo-polyfills-css-shim" */ './polyfills/css-shim.js').then(() => {
...
export const patchBrowser = (): Promise<d.CustomElementsDefineOptions> => {
  ...
  if (!BUILD.safari10 && importMeta !== '') {
    ...
    if (BUILD.dynamicImportShim && !win.customElements) {
      ...
      // @ts-ignore
      return import(/* webpackChunkName: "rindo-polyfills-dom" */ './polyfills/dom.js').then(() => opts);
    }
  }

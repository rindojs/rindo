...
const hostRefs: WeakMap<d.RuntimeRef, d.HostRef> = /*@__PURE__*/ BUILD.hotModuleReplacement
  ? ((window as any).__RINDO_HOSTREFS__ ||= new WeakMap())
  : new WeakMap();
...
/**
 * Register a host element for a Rindo component, setting up various metadata
 ...
 *
 * Background:
 * During component init, Rindo loops through any `@Prop()` or `@State()` decorated properties
 ...
 * With modern JS, compiled classes (e.g. `target: 'es2022'`) compiled Rindo components went from:
 *
 * ```ts
 * class MyComponent {
 *   constructor() {
...
const reWireGetterSetter = (instance: any, hostRef: d.HostRef) => {
  ...
  members.map(([memberName, [memberFlags]]) => {
    if (
      ...
      // Get the original Rindo prototype `get` / `set`

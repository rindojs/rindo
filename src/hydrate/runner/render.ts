...
import { MockWindow, serializeNodeToHtml } from '@rindo/core/mock-doc';
...
async function render(win: MockWindow, opts: HydrateFactoryOptions, results: HydrateResults) {
  if ('process' in globalThis && typeof process.on === 'function' && !(process as any).__rindoErrors) {
    (process as any).__rindoErrors = true;

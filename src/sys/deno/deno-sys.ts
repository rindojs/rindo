...
export function createDenoSys(c: { Deno?: any } = {}) {
  ...
  let rindoBaseUrl: URL;
  let rindoRemoteUrl: string;
  let rindoExePath = new URL(`../../compiler/rindo.js`, import.meta.url).href;
  ...
  const sys: CompilerSystem = {
    ...
    async ensureDependencies(opts) {
      ...
      const rindoDep = opts.dependencies.find(dep => dep.name === '@rindo/core');
      const typescriptDep = opts.dependencies.find(dep => dep.name === 'typescript');

      rindoRemoteUrl = new URL(`../../compiler/rindo.js`, import.meta.url).href;
      if (!isRemoteUrl(rindoRemoteUrl)) {
        rindoRemoteUrl = sys.getRemoteModuleUrl({ moduleId: rindoDep.name, version: rindoDep.version, path: rindoDep.main });
      }
      rindoBaseUrl = new URL(`../../`, rindoRemoteUrl);
      rindoExePath = sys.getLocalModulePath({ rootDir: opts.rootDir, moduleId: rindoDep.name, path: rindoDep.main });
      ...
      const ensureRindo = fetchWrite(diagnostics, rindoRemoteUrl, rindoExePath);
      const ensureTypescript = fetchWrite(diagnostics, typescriptRemoteUrl, typescriptExePath);

      await Promise.all([ensureRindo, ensureTypescript]);

      sys.getCompilerExecutingPath = () => rindoExePath;

      timespan.finish(`ensure dependencies end`);

      return {
        rindoPath: rindoExePath,
    ...
    async ensureResources(opts) {
      ...
      const rindoPkg = sys.getLocalModulePath({ rootDir: opts.rootDir, moduleId: rindoDep.name, path: 'package.json' });
      const typescriptPkg = sys.getLocalModulePath({ rootDir: opts.rootDir, moduleId: typescriptDep.name, path: 'package.json' });

      const rindoCheck = sys.access(rindoPkg);
      const typescriptCheck = sys.access(typescriptPkg);

      const rindoResourcesExist = await rindoCheck;
      const typescriptResourcesExist = await typescriptCheck;

      if (!rindoResourcesExist) {
        opts.logger.debug(`rindoBaseUrl: ${rindoBaseUrl.href}`);
        rindoDep.resources.forEach(p => {
          deps.push({
            url: new URL(p, rindoBaseUrl).href,
            path: sys.getLocalModulePath({ rootDir: opts.rootDir, moduleId: rindoDep.name, path: p }),
          });
        });
      }

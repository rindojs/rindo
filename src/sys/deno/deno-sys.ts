  ...
  const sys: CompilerSystem = {
    ...
    async ensureDependencies(opts) {
      ...
      if (!isRemoteUrl(rindoRemoteUrl)) {
        rindoRemoteUrl = sys.getRemoteModuleUrl({
          moduleId: rindoDep.name,
          version: rindoDep.version,
          path: rindoDep.main,
        });
      }
      rindoBaseUrl = new URL(`../../`, rindoRemoteUrl);
      rindoExePath = sys.getLocalModulePath({
    ...
    async ensureResources(opts) {
      ...
      const rindoPkg = sys.getLocalModulePath({
        rootDir: opts.rootDir,
        moduleId: rindoDep.name,
        path: 'package.json',
      });

    ...
    if (!isDestroyed) {
      ...
      if (Array.isArray(msgsFromWorker)) {
        for (const msgFromWorker of msgsFromWorker) {
          if (msgFromWorker) {
            const task = tasks.get(msgFromWorker.rindoId);
            if (task) {
              tasks.delete(msgFromWorker.rindoId);
              if (msgFromWorker.rindoRtnError) {
            ...
            } else if (msgFromWorker.rindoRtnError) {
              console.error(msgFromWorker.rindoRtnError);
            }
          }
        }
      }
  ...
  const send = (...args: any[]) =>
    new Promise<any>((resolve, reject) => {
      if (isDestroyed) {
        ...
        times.set(msg.rindoId, Date.now());

        tasks.set(msg.rindoId, {
          resolve,
          reject,
        });

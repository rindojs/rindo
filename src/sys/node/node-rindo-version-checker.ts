...
const CHANGELOG = `https://github.com/rindojs/rindo/blob/main/CHANGELOG.md`;
...
function getLastCheckStoragePath() {
  return path.join(tmpdir(), 'rindo_last_version_node.json');
}

function printUpdateMessage(logger: Logger, currentVersion: string, latestVersion: string) {
  const installMessage = `npm install @rindo/core`;
  const msg = [

...
export function loadConfigFile(fs: d.FileSystem, configPath: string, process?: NodeJS.Process) {
  ...
  if (typeof configPath === 'string') {
    if (!path.isAbsolute(configPath)) {
      throw new Error(`Rindo configuration file "${configPath}" must be an absolute path.`);
    }
  ...
  if (hasConfigFile) {
    ...
    if (!configFileData.config) {
      throw new Error(`Invalid Rindo configuration file "${configPath}". Missing "config" property.`);
    }

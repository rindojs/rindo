...
import { checkVersion } from './node-rindo-version-checker';
import { NodeWorkerController } from './node-worker-controller';

/**
 * Create a node.js-specific {@link CompilerSystem} to be used when Rindo is
...
export function createNodeSys(c: { process?: any } = {}): CompilerSystem {
  ...
  const compilerExecutingPath = path.join(__dirname, '..', '..', 'compiler', 'rindo.js');
  ...
  const sys: CompilerSystem = {
    ...
    async ensureDependencies() {
      ...
      return {
        rindoPath: sys.getCompilerExecutingPath(),
        diagnostics: [],
      };
    },
    ...
    removeDir(p, opts) {
      return new Promise((resolve) => {
        ...
        if (recursive) {
          // TODO: In a future version of Node, `force: true` will be required in the options argument. At
          // the time of this writing, Rindo's Node typings do not support this option.
    ...
    setupCompiler(c) {
       ...
       * implementation that is used when running Rindo's compiler in "watch mode" in Node.
       *

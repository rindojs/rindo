...
/**
 * Patches the `cloneNode` method on a `scoped` Rindo component.
 *
 * @param HostElementPrototype The Rindo component to be patched
 */
  ...
  HostElementPrototype.cloneNode = function (deep?: boolean) {
    ...
    if (BUILD.slot && !isShadowDom && deep) {
      ...
      for (; i < childNodes.length; i++) {
        ...
        nonRindoNode = rindoPrivates.every((privateField) => !(childNodes[i] as any)[privateField]);
        if (slotted) {
...
/**
 * Patches the `appendChild` method on a `scoped` Rindo component.
...
/**
 * Patches the `removeChild` method on a `scoped` Rindo component.
 ...
 *
 * @param ElementPrototype The Rindo component to be patched
 */
...
const patchInsertBefore = (HostElementPrototype: HTMLElement) => {
  ...
  HostElementPrototype.insertBefore = function <T extends d.PatchedSlotNode>(
     ...
     * allowing Rindo to properly manage the slot placement.
     */

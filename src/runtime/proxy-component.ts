    ...
    if (BUILD.observeAttribute && (!BUILD.lazyLoad || flags & PROXY_FLAGS.isElementConstructor)) {
      ...
      prototype.attributeChangedCallback = function (attrName: string, oldValue: string, newValue: string) {
        plt.jmp(() => {
          ...
          //
          //  TODO(RINDO-16) we should think about whether or not we actually want to be reflecting the attributes to
          ...
          if (this.hasOwnProperty(propName)) {
            ...
            // if the propName exists on the prototype of `Cstr`, this update may be a result of Rindo using native
      ...
      // on a component as well as any Rindo-specific "members" (`@Prop()`s and `@State()`s).

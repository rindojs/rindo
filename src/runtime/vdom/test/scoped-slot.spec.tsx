...
describe('scoped slot', () => {
  ...
  it('should relocate nested named slot nodes', async () => {
    ...
    const { root } = await newSpecPage({
      components: [CmpA],
      html: `<fml-test><tiger slot="start">88</tiger></fml-test>`,
    });
  ...
  it('no content', async () => {
    @Component({ tag: 'fml-parent', scoped: true })
    class Parent {
      render() {
        return (
          <lion>
    ...
    @Component({ tag: 'fml-child', scoped: true })
    class Child {
      render() {
        return <slot></slot>;
      }
    ...
    const { root } = await newSpecPage({
      ...
      html: `<fml-parent></fml-parent>`,
    });

    expect(root.children).toHaveLength(1);
    expect(root.firstElementChild.nodeName).toBe('LION');
    expect(root.firstElementChild.firstElementChild.nodeName).toBe('FML-CHILD');
  ...
  it('should relocate parent content after child content dynamically changes slot wrapper tag', async () => {
    ...
    class Parent {
      ...
      render() {
        return <fml-child>{this.innerH}</fml-child>;
      }
    }
    ...
    expect(root.firstElementChild.nodeName).toBe('FML-CHILD');
    ...
    const child = root.querySelector('fml-child');
    child.Tag = 'article';
    await waitForChanges();

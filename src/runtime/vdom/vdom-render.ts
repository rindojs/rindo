...
const createElm = (oldParentVNode: d.VNode, newParentVNode: d.VNode, childIndex: number) => {
  ...
  if (BUILD.isDev && newVNode.$elm$) {
    consoleDevError(
      ...
      } node should not be shared within the same renderer. The renderer caches element lookups in order to improve performance. However, a side effect from this is that the exact same JSX node should not be reused. For more information please see https://rindojs.web.app/docs/templating-jsx#avoid-shared-jsx-nodes`,
    );
  }
  ...
  if (BUILD.vdomText && newVNode.$text$ !== null) {
    ...
    if (!win.document) {
      throw new Error(
        "You are trying to render a Rindo component in an environment that doesn't support the DOM. " +
 ...
 * to the root of the Rindo component currently being rendered. This happens when a parent
...
render() {
  ...
  // with Rindo components to override e.g. the `role` attribute on a
  ...
  if (isInitialLoad && rootVnode.$attrs$) {
    for (const key of Object.keys(rootVnode.$attrs$)) {
      ...
      // are present.
      //
      // Likewise, `ref` and `key` are special internal values for the Rindo
    ...
    if (checkSlotRelocate) {
      ...
      for (const relocateData of relocateNodes) {
        ...
        if (slotRefNode) {
          ...
          //
          // TODO(RINDO-914): Remove `experimentalSlotFixes` check
          if (

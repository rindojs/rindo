import { Component, Event, EventEmitter, Listen, Method, State } from '@rindo/core';
import { newSpecPage } from '@rindo/core/testing';

describe('event', () => {
  it('event normal fmlChange event', async () => {
    @Component({ tag: 'cmp-a' })
    class CmpA {
      @Event() fmlChange: EventEmitter;
      ...
      @Method()
      emitEvent() {
        const event = this.fmlChange.emit();
        expect(event.type).toEqual('fmlChange');
      }
    ...
    let called = false;
    page.root.addEventListener('fmlChange', (ev: CustomEvent) => {
  ...
  it('should have custom name', async () => {
    @Component({ tag: 'cmp-a' })
    class CmpA {
      @Event({ eventName: 'fmlStyle' }) style: EventEmitter;
      @State() counter = 0;
    ...
    let called = false;
    root.addEventListener('fmlStyle', (ev: CustomEvent) => {
      expect(ev.bubbles).toBe(true);
      expect(ev.cancelable).toBe(true);
      expect(ev.composed).toBe(true);
      called = true;

import { Component, Element, forceUpdate, getRenderingRef, h, Host, Prop, setErrorHandler, State } from '@rindo/core';
...
describe('render-vdom', () => {
  ...
  it('should error when reusing vnodes', async () => {
    ...
    try {
      await newSpecPage({
        components: [CmpA],
        html: `<cmp-a first="Rindo" last="'Don't call me a framework' JS"></cmp-a>`,
      });
    } catch (e) {
      error = e;
    }
  ...
  describe('ref property', () => {
    ...
    it('should not call ref cb w/ null when children w/ keys are reordered', async () => {
      ...
      // because without the `key` Rindo's child-identity heuristic falls over.
      @Component({ tag: 'cmp-a' })
      class CmpA {
        divRef: HTMLElement;
        @Prop() state = true;

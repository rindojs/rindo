...
export async function run(process: NodeJS.Process, sys: d.RindoSystem, logger: d.Logger) {
  ...
  try {
    ...
    if (flags.config && !sys.fs.existsSync(configPath)) {
      throw new Error(`Rindo configuration file cannot be found at: "${flags.config}"`);
    }
  ...
  try {
    ...
    process.title = `Rindo: ${config.namespace}`;

...
export const taskTest = async (config: ValidatedConfig): Promise<void> => {
  ...
  try {
    /**
     * We dynamically import the testing submodule here in order for Rindo's lazy module checking to work properly.
     *
     ...
     * it was installed).
     */
    const { createTesting } = await import('@rindo/core/testing');

...
export async function newE2EPage(opts: NewE2EPageOptions = {}): Promise<E2EPage> {
  ...
  try {
    ...
    page.debugger = () => {
      if (env.__RINDO_E2E_DEVTOOLS__ !== 'true') {
...
async function e2eGoTo(page: E2EPageInternal, url: string, options: WaitForOptions = {}) {
  ...
  const browserUrl = env.__RINDO_BROWSER_URL__;
...
async function waitForRindo(page: E2EPage, options: WaitForOptions) {
  ...
  try {
    await page.waitForFunction('window.rindoAppLoaded', { timeout });
  } catch (e) {
    throw new Error(`App did not load within ${timeout}ms. Please ensure the content loads a rindo application.`);
  }
}

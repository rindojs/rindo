...
export class Testing implements d.Testing {
  ...
  constructor(config: d.Config) {
    ...
    if (this.isValid) {
      if (!config.flags.spec && !config.flags.e2e) {
        config.logger.error(`Testing requires either the --spec or --e2e command line flags, or both. For example, to run unit tests, use the command: rindo test --spec`);
        this.isValid = false;
      }
    }
  }
  ...
  async runTests() {
    ...
    if (config.flags.e2e) {
      ...
      if (this.devServer) {
        env.__RINDO_BROWSER_URL__ = this.devServer.browserUrl;
        this.config.logger.debug(`e2e dev server url: ${env.__RINDO_BROWSER_URL__}`);

        env.__RINDO_APP_URL__ = getAppUrl(config, this.devServer.browserUrl);
        this.config.logger.debug(`e2e app url: ${env.__RINDO_APP_URL__}`);
      }
    }

...
import { getRunner, getScreenshot } from './jest/jest-rindo-connector';
...
export const createTesting = async (config: ValidatedConfig): Promise<Testing> => {
  ...
  /**
   * Initiate running spec and/or end-to-end tests with Rindo
    ...
    try {
      ...
      if (doScreenshots) {
        ...
        if (config.testing.screenshotTimeout != null) {
          env.__RINDO_SCREENSHOT_TIMEOUT_MS__ = config.testing.screenshotTimeout.toString();
        }
      ...
      if (opts.e2e) {
        ...
        if (devServer) {
          ...
          if (styleUrl) {
            env.__RINDO_APP_STYLE_URL__ = styleUrl;
...
/**
 * Create a Rindo configuration for testing purposes.
 *

...
/**
 * For Rindo's purposes, an instance of a Jest/Puppeteer environment only needs to have a handful of functions.
 * This does not mean that Jest does not require additional functions on an environment. However, those requirements may
 * change from version-to-version of Jest. Rindo overrides the functions below, and with our current design of
...
/**
 * For Rindo's purposes, an instance of a Jest `TestRunner` only needs to have an async `runTests` function.
 * This does not mean that Jest does not require additional functions. However, those requirements may change from
 * version-to-version of Jest. Rindo overrides the `runTests` function, and with our current design of integrating
 ...
 * 1. It allows Rindo to have a single source of truth for the return type(s) on {@link JestFacade} (and its
 *    implementations)
 * 2. It prevents TypeScript from expanding Rindo type declarations in the generated `.d.ts` file. This is necessary
 *    as TypeScript will make assumptions about where it can dynamically resolve Rindo typings from, which are not
...
/**
 * This type serves as an alias for a function that invokes Rindo's Screenshot runner.
 *
 * This alias serves two purposes:
 * 1. It allows Rindo to have a single source of truth for the return type(s) on {@link JestFacade} (and its
 *    implementations)
 ...
 *    true for every type of Rindo project directory structure.
 */
export type JestPresetConfig = Config.InitialOptions;

/**
 * Get the current major version of Jest that Rindo reconciles
 *

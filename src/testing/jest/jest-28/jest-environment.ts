...
import type { E2EProcessEnv, JestEnvironmentGlobal } from '@rindo/core/internal';
...
export function createJestPuppeteerEnvironment(): JestPuppeteerEnvironmentConstructor {
  const JestEnvironment = class extends NodeEnvironment {
    // TODO(RINDO-1023): Remove this @ts-expect-error
    // @ts-expect-error - Rindo's Jest environment adds additional properties to the Jest global, but does not extend it
    ...
    override async setup() {
      if ((process.env as E2EProcessEnv).__RINDO_E2E_TESTS__ === 'true') {
     ...
     * Jest-Rindo screenshot test, so that we may accurately report on it.
     *
    ...
    async handleTestEvent(event: Circus.AsyncEvent): Promise<void> {
      ...
      if (event.name === 'test_start') {
         ...
         * While this does not necessarily guarantee the generated name will be unique, it matches previous Jest-Rindo
         * screenshot behavior.
         */
    ...
    async newPuppeteerPage() {
      ...
      if (typeof env.__RINDO_DEFAULT_TIMEOUT__ === 'string') {
        page.setDefaultTimeout(parseInt(env.__RINDO_DEFAULT_TIMEOUT__, 10));
      }
      return page;
    }

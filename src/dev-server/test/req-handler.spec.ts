...
describe('request-handler', () => {
  ...
  beforeEach(async () => {
    ...
    const rindoConfig = validated.config;
    rindoConfig.flags.serve = true;

    rindoConfig.devServer = {
      devServerDir: normalizePath(path.join(__dirname, '..')),
      root: normalizePath(path.join(root, 'www')),
      basePath: '/',
    };

    await sys.createDir(rindoConfig.devServer.root);
    await sys.writeFile(path.join(rindoConfig.devServer.devServerDir, 'templates', 'directory-index.html'), tmplDir);

    devServerConfig = validateDevServer(rindoConfig, []);
    req = {} as any;
    res = {} as any;
